...
func runJob(resultChannel chan int, i Job) {
	resultChannel <- longCalculation(i)
}
...
func main() {
	...
	for i := 0; i < len(jobs); i++ {
		go runJob(resultChannel, jobs[i])
	}
	//   - Sum the results from each job to generate a final result, and print it
	//     to the terminal
	resultSum := 0
	for i := 0; i < len(jobs); i++ {
		resultSum += <-resultChannel
	}
	fmt.Println("Sum is:", resultSum)
}
